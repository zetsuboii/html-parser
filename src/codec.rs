...
impl HtmlParser {
    pub fn tokenize<'a>(data: &'a str) -> Result<Vec<HtmlAst<'a>>, HtmlError> {
    ...
    pub fn parse<'a>(data: &'a str) -> Result<Vec<HtmlElement>, HtmlError> {
        let mut reader = StrReader::new(data);
    ...
    pub fn encode(elements: Vec<HtmlElement<'_>>) -> String {
        ...
        for element in elements {
            if element.attributes.len() > 0 {
                ...
                if let Some(text) = element.inner_text {
                    ...
                    html.push_str(&Self::encode(element.children));
                }
...
mod tests {
    ...
    fn single_tag_tokenize() {
        ...
        let tokens = HtmlParser::tokenize(html).unwrap();
        assert_eq!(
            tokens,
            vec![
    ...
    fn decode_html() {
        ...
        let element = HtmlParser::parse(html).unwrap();
        assert_eq!(
            element,
            vec![HtmlElement {
                tag: "div",
    ...
    fn encode_html() {
        ...
        let html = HtmlParser::encode(elements);
